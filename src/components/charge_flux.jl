...
function update_ct_acceptor_charges_with_charge_flux!(
    coords::AbstractVector{MVector{3, Float64}},
    labels::AbstractVector{String},
    fragment_indices::AbstractVector{Vector{Int}},
    charges::AbstractVector{Float64},
    ...
    for i_frag in eachindex(fragment_indices)
        if length(fragment_indices[i_frag]) == 3
            ...
            dq_H1 = (
                j_OH_ct * (r_OH1_length - r_OH_eq)
            )
            dq_H2 = (
                j_OH_ct * (r_OH2_length - r_OH_eq)
            )
            dq_O  = -(dq_H1 + dq_H2)
            
            # We basically have to write the charge flux in this way
            # since we want to be able move information between the atoms
            # to convey the force from hydrogen to oxygen.
            charges[i_O]  += dq_O
            charges[i_H1] += dq_H1
            charges[i_H2] += dq_H2
